...
در این مسیر همان صفحه معرفی نمایش داده می‌شود. دکمه «متوجه شدم؛ ورود به پنل» پنل واقعی را در مسیر زیر باز می‌کند:

```text
https://YOUR-DOMAIN/gucci/panel/
```
...
تمام پیش‌فرض‌های ضروری داخل Image قرار دارند: پنل روی پورت `1`، Landing روی `/gucci/`، پنل واقعی روی `/gucci/panel/`، ورود اولیه `gucci / gucci`، دیتابیس در `/data/x-ui`، Xray و Subscription اصلی 3X-UI. بعد از باز شدن پنل، تمام تنظیمات شبکه و Inboundها در اختیار خود مدیر است.
