...
نسخه عمومی و Fork-ready پنل سنایی برای Railway. طبق درخواست، سورس پنل روی Release رسمی **3X-UI v3.4.2** و Xray روی `v26.6.27` Pin شده است؛ ارتقای زمان‌بندی‌شده به نسخه‌های جدیدتر غیرفعال است. تمام فایل‌های لازم داخل همین Repository است و برای بالا آمدن پنل هیچ Variable یا سرویس جانبی لازم نیست.
...
در این مسیر همان صفحه معرفی نمایش داده می‌شود. دکمه «متوجه شدم → ورود به پنل» با یک مجوز کوتاه‌مدت، پنل واقعی را روی همان آدرس `/gucci/` باز می‌کند و کاربر لازم نیست URL دیگری وارد کند.
```
...
برای سازگاری بیشتر با Xray، Mihomo/Clash Meta، sing-box و کلاینت‌های قدیمی‌تر، Image از Xray-core `v26.6.27` استفاده می‌کند؛ این نسخه پیش از محدودیت پیش‌فرض جدید REALITY است. گزارش فنی و تست A/B کامل در [`COMPATIBILITY_3_4_2_TO_3_6_0.md`](COMPATIBILITY_3_4_2_TO_3_6_0.md) ثبت شده است.
...
| `GUCCI_HEALTH_TCP_PORTS` | Comma-separated local TCP Proxy targets checked by `/healthz`; repeated failures trigger Railway recovery | — |
